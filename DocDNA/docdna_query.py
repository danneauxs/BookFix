...
def build_parser():
    ...
    p.add_argument("project", help="Path to the project root (contains DocDNA/)")
...
def mcp_serve(project: str):
    """Start MCP server loop reading JSON-RPC from stdin."""
    import sys
    ...
    print(json.dumps({
        ...
        "result": {
            ...
            "serverInfo": {"name": "docdna-query", "version": "1.0.0"},
            "capabilities": {
                "tools": {
                    "docdna_overview": {"description": "Project overview with stats, files, modules"},
                    "docdna_function": {
                        "description": "Function details by name",
                        "inputSchema": {
                            "type": "object",
                            "properties": {
                    ...
                    "docdna_search": {
                        "description": "FTS5 full-text search across all knowledge",
                        "inputSchema": {
                            "type": "object",
                            "properties": {
                    ...
                    "docdna_faq": {
                        "description": "Search project FAQs",
                        "inputSchema": {
                            "type": "object",
                            "properties": {
                    ...
                    "docdna_where_used": {
                        "description": "All references to a function/class",
                        "inputSchema": {
                            "type": "object",
                            "properties": {
                    ...
                    "docdna_dead_code": {
                        "description": "List functions with zero callers"
                    },
                    "docdna_impact": {
                        "description": "Reverse transitive callers (dependents)",
                        "inputSchema": {
                            "type": "object",
                            "properties": {
                                "name": {"type": "string", "description": "Function name"},
                    ...
                    "docdna_imports": {
                        "description": "Import resolution for a symbol",
                        "inputSchema": {
                            "type": "object",
                            "properties": {
                                "name": {"type": "string", "description": "Module or symbol name"}
                            },
                            "required": ["name"]
                        }
                    },
    ...
    for line in sys.stdin:
        ...
        try:
            req = json.loads(line)
            req_id = req.get("id")
            method = req.get("params", {}).get("name", "")
            args_in = req.get("params", {}).get("arguments", {})

            if method == "docdna_overview":
                result = cmd_overview(cur, argparse.Namespace(json=True, limit=50))
            elif method == "docdna_function":
                result = cmd_function(cur, args_in.get("name", ""), argparse.Namespace(json=True, limit=5))
            elif method == "docdna_search":
                result = cmd_search(cur, args_in.get("query", ""), argparse.Namespace(json=True, limit=20))
            elif method == "docdna_callgraph":
                result = cmd_callgraph(cur, args_in.get("name", ""), args_in.get("depth", 1),
                                       argparse.Namespace(json=True, limit=50))
            elif method == "docdna_file":
                result = cmd_file(cur, args_in.get("path", ""), argparse.Namespace(json=True, limit=50))
            elif method == "docdna_faq":
                result = cmd_faq(cur, args_in.get("topic", ""), argparse.Namespace(json=True, limit=20))
            elif method == "docdna_where_used":
                result = cmd_where_used(cur, args_in.get("name", ""), argparse.Namespace(json=True, limit=50))
            elif method == "docdna_dead_code":
                result = cmd_dead_code(cur, argparse.Namespace(json=True, limit=50))
            elif method == "docdna_impact":
                result = cmd_impact(cur, args_in.get("name", ""), argparse.Namespace(json=True, depth=args_in.get("depth", 3), limit=50))
            elif method == "docdna_imports":
                result = cmd_imports(cur, args_in.get("name", ""), argparse.Namespace(json=True, limit=50))
            else:
                result = json.dumps({"error": f"Unknown tool: {method}"})

            if req_id:
                print(json.dumps({"jsonrpc": "2.0", "id": req_id, "result": json.loads(result) if isinstance(result, str) else result}), flush=True)
...
def main():
    ...
    # MCP mode
    if args.project == "--mcp":
        # Expect DOCDNA_ROOT env var
        project = os.environ.get("DOCDNA_ROOT", "")
        if not project:
            sys.exit("Error: DOCDNA_ROOT env var required for --mcp mode")
        mcp_serve(project)
        return
