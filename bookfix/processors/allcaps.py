...
class AllCapsProcessor:
    ...
    def process_all_caps_sequences(self, ctx: "BookfixContext") -> "BookfixContext":
        ...
        if not hasattr(ctx, "lowercase_set"):
            ctx.lowercase_set = (
                set(ctx.upper_to_lower) if hasattr(ctx, "upper_to_lower") else set()
            )
        ...
        log_message(
            f"Original text length: {len(original_for_detection)} chars", level="DEBUG"
        )

        # Compile regex (no newlines, uppercase & spaces only)
        sequence_pattern = re.compile(r"\b[A-Z](?:[A-Z ]*[A-Z])\b")
        log_message(
            f"Using sequence_pattern: {sequence_pattern.pattern}", level="DEBUG"
        )

        # Detect sequences in the original text
        self.all_caps_matches_original = list(
            sequence_pattern.finditer(original_for_detection)
        )
    ...
    def _process_next_sequence(self, ctx: "BookfixContext"):
        ...
        while self.current_match_index < len(self.all_caps_matches_original):
            ...
            log_message(
                f"DEBUG: Checking sequence '{seq_text}' against ignore_set: {ctx.ignore_set}",
                level="DEBUG",
            )
            ...
            if seq_text in ctx.ignore_set or seq_text in self.decided_sequences_text:
                if seq_text in ctx.ignore_set:
                    log_message(
                        f"Skipping ignored sequence '{seq_text}' (found in ignore_set)",
                        level="DEBUG",
                    )
                else:
                    log_message(
                        f"Skipping already decided sequence '{seq_text}'", level="DEBUG"
                    )
            ...
            log_message(
                f"Processing sequence '{seq_text}' at span {span}", level="DEBUG"
            )
    ...
    def _apply_highlighting(self, start: int, end: int, word: str):
        ...
        if start < 0 or end > len(current_text) or start >= end:
            log_message(
                f"Invalid highlight range: {start}-{end} (text length: {len(current_text)})"
            )
            return
    ...
    def handle_caps_choice(self, choice: str, ctx: "BookfixContext") -> bool:
        ...
        log_message(
            f"[PATCH ACTIVE] handle_caps_choice() got choice={choice!r}", level="DEBUG"
        )
        ...
        if choice.lower() in ("y", "yes", "0"):  # '0' for PyQt5 button index
            ...
            log_message(
                f"Unknown choice '{choice}' in handle_caps_choice()", level="WARNING"
            )
            return True
        ...
        self.current_match_index += 1
        log_message(
            f"Choice handled for '{seq}' → '{choice}'. Moving on.", level="DEBUG"
        )
